document.addEventListener("DOMContentLoaded", function () {
    ...
    function openAvisoModal(avisoData = null) {
        ...
        if (avisoData) {
            document.getElementById("avisoModalTitle").textContent = "Editar Aviso";
            document.getElementById("avisoId").value = avisoData.id;
            document.getElementById("contenido").value = avisoData.contenido;
            // Asegúrate que el formato de fecha sea el correcto para el input datetime-local
            document.getElementById("fecha_inicio").value = avisoData.fechaInicio;
            document.getElementById("fecha_fin").value = avisoData.fechaFin;
            document.getElementById("avisoMethod").value = "PUT";
        } else {
            document.getElementById("avisoModalTitle").textContent = "Añadir Nuevo Aviso";
            ...
            document.getElementById("fecha_fin").value = endDate.toISOString().slice(0, 16);
        }
    ...
    editAvisoButtons.forEach((button) => {
        button.addEventListener("click", function () {
            ...
            const avisoData = {
                ...
                contenido: avisoItem.querySelector("p").textContent.replace(/^-\s*/, "").trim(),
