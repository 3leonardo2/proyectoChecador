...
<body>
    ...
    <div class="main-container">
        <form action="{{ route('instituciones.update', $institucion->id_institucion) }}" method="POST" class="registrar_insti-wrapper" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="institucion-profile-section">
                    <div class="profile-image-container">
                        @if($institucion->imagen)
                            <img src="{{ asset('storage/'.$institucion->imagen) }}" alt="ImagenInstitución" class="profile-image">
                        @endif
                    </div>
            ...
            <div class="institucion-scrollable-content">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nombre_ins" name="nombre" placeholder="Nombre..." required value="{{ old('nombre', $institucion->nombre) }}">
                </div>
                <div class="form-group">
                    <label for="direccion_ins">Dirección*:</label>
                    <input type="text" id="direccion_ins" name="direccion" placeholder="Dirección..." required value="{{ old('direccion', $institucion->direccion) }}">
                </div>
                <div class="form-group">
                    <label for="telefono_ins">Teléfono de contacto*:</label>
                    <input type="text" id="telefono_ins" name="telefono" placeholder="Teléfono..." value="{{ old('telefono', $institucion->telefono) }}">
                </div>
                <div class="form-group">
                    <label for="correo_ins">Correo electrónico*:</label>
                    <input type="email" id="correo_ins" name="correo" placeholder="Correo electrónico..." value="{{ old('correo', $institucion->correo) }}">
                </div>
                ...
                <div id="carreras-container">
                    ...
                    <div class="carrera-block" data-index="{{ $index }}">
                        <input type="hidden" name="carreras[{{ $index }}][id_carrera]" value="{{ $carrera->id_carrera }}">
                        <div class="form-group">
                            <label for="nombre_carr_{{ $index }}">Nombre:</label>
                            <input type="text" id="nombre_carr_{{ $index }}" name="carreras[{{ $index }}][nombre_carr]" placeholder="Nombre de carrera..." value="{{ old('carreras.'.$index.'.nombre_carr', $carrera->nombre_carr) }}">
                        </div>
                        <div class="form-group">
                            <label for="gerente_carr_{{ $index }}">Gerente:</label>
                            <input type="text" id="gerente_carr_{{ $index }}" name="carreras[{{ $index }}][gerente_carr]" placeholder="Gerente de carrera..." value="{{ old('carreras.'.$index.'.gerente_carr', $carrera->gerente_carr) }}">
                        </div>
                        <div class="form-group">
                            <label for="telefono_carr_{{ $index }}">Teléfono:</label>
                            <input type="text" id="telefono_carr_{{ $index }}" name="carreras[{{ $index }}][telefono_carr]" placeholder="Teléfono de carrera..." value="{{ old('carreras.'.$index.'.telefono_carr', $carrera->tel_gerente) }}">
                        </div>
                        <div class="form-group">
                            <label for="correo_carr_{{ $index }}">Correo:</label>
                            <input type="email" id="correo_carr_{{ $index }}" name="carreras[{{ $index }}][correo_carr]" placeholder="Correo de carrera..." value="{{ old('carreras.'.$index.'.correo_carr', $carrera->correo_carr) }}">
                ...
                <div class="form-actions">
                    ...
                    <button type="button" class="cancel-button" onclick="window.location='{{ route('instituciones.index') }}'">Cancelar</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        function eliminarCarrera(element) {
            ...
            if (block.querySelector('input[name*="[id_carrera]"]')) {
                ...
                hiddenInput.name = block.querySelector('input[name*="[id_carrera]"]').name.replace('id_carrera', '_destroy');
                hiddenInput.value = '1';
